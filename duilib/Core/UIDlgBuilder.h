...
namespace DuiLib {
	...
	public:
		template<typename T>
		T* Create(STRINGorID xml, LPCTSTR type = NULL, IDialogBuilderCallback* pCallback = NULL,
			CPaintManagerUI* pManager = NULL, CControlUI* pParent = NULL) {
			return (T*)Create(xml, type,pCallback, pManager, pParent);
		}
		template<typename T>
		T* Create(IDialogBuilderCallback* pCallback = NULL, CPaintManagerUI* pManager = NULL,
			CControlUI* pParent = NULL) {
			return (T*)Create(pCallback, pManager, pParent);
		}
	public:
		CDialogBuilder();
