...
namespace DuiLib {
	...
	CControlUI* CListUI::GetItemAtByTag(const UINT_PTR& tag) const {
		return m_pList->GetItemAtByTag(tag);
	}
	CControlUI* CListUI::GetItemAtByTagINT64(const INT64& tag) const {
		return m_pList->GetItemAtByTagINT64(tag);
	}
	CControlUI* CListUI::GetItemAtByTagUINT64(const UINT64& tag) const {
		return m_pList->GetItemAtByTagUINT64(tag);
	}
	...
	BOOL CListUI::SortItems(const PULVCompareFunc& pfnCompare, UINT_PTR dwData)
	{
		if (!m_pList)
			return FALSE;
	...
	BOOL CListBodyUI::SortItems(const PULVCompareFunc& pfnCompare, UINT_PTR dwData)
	{
		if (!pfnCompare)
			return FALSE;
...
#endif
		std::vector<CControlUI*> Targets;

		for (int i = 0; i < m_items.GetSize(); ++i)
		{
			Targets.emplace_back((CControlUI*)m_items.GetAt(i));
		}
		if (Targets.empty())
			return FALSE;
		std::sort(pData, pData+ m_items.GetSize(),
			[&](const auto& item1, const auto& item2)->int
			{
				return m_pCompareFunc((UINT_PTR)item1, (UINT_PTR)item2, m_compareData);
			});

		for (int i = 0; i < m_items.GetSize(); ++i)
		{
			auto pItem = (IListItemUI*)(static_cast<CControlUI*>(m_items[i])->GetInterface(TEXT("ListItem")));
			if (pItem)
			{
				pItem->SetIndex(i);
				pItem->Select(false);
...
#if 0
				{
					auto parent = (CListUI*)GetParent()->GetParent();
					if (parent)
					{
						auto nCurSelColumn = parent->GetItemIndex(this);
						auto sk = 0;

						auto head = parent->GetHeader();
						if (head)
						{
							auto itemat = head->GetItemAt(nCurSelColumn);
							if (itemat->GetTag() == 0)
								itemat->SetTag(1);
							else
								itemat->SetTag(0);
							...
							parent->SortItems(
								[](UINT_PTR param1, UINT_PTR param2, UINT_PTR param3)->BOOL
								{
									CControlUI* target = (CControlUI*)param3;
									auto listAt1 = (CListContainerElementUI*)param1;
									auto listAt2 = (CListContainerElementUI*)param2;

									if (listAt1->GetClass() == _T("ListTextElementUI") && listAt2->GetClass() == _T("ListTextElementUI"))
									{
										auto sk = 0;
									}
									else if (listAt1->GetClass() == _T("ListContainerElementUI") && listAt2->GetClass() == _T("ListContainerElementUI"))
									{
										auto pElem = (CListContainerElementUI*)listAt1;
										auto sk = 0;
									}

									auto asdlkfsdfj = listAt1->GetClass();
	...
	void CListTextElementUI::SetText(int iIndex, LPCTSTR pstrText,DWORD dwTextColor)
	{
		if (m_pOwner == NULL) return;
		...
		if (iIndex == pInfo->nColumns) {
			pInfo->dwTextColor = dwTextColor;
		}
